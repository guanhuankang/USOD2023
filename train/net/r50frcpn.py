...
class R50FrcPN(nn.Module):
    ...
    def forward(self, x, global_step=0.0, **kwargs):
        ...
        attn, loss = self.sal(self.conv(out[-1]))
        y = self.head(out[0])
        ...
        if self.training:
            ...
            if alpha_bce>1e-3 and alpha_bce<0.999:
                ...
                loss += bceloss * alpha_bce; loss_dict.update({"alpha_bce": alpha_bce})
            if alpha_other>1e-3:
                ...
                loss += (lwtloss * w[0] + uncerloss * w[1]) * alpha_other; loss_dict.update({"alpha_other": alpha_other})
